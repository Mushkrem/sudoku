...
void draw_board() {
    ...
    for (int column = 0; column <= board_size_x; column++) {
        //Interface.draw_text((COORD) { x_offset + column, y_offset - 2 }, L"%x", column);
        for (int row = 0; row <= board_size_y; row++) {
            

            if (column == 0) { // Left side
                Interface.draw_text((COORD) { x_offset, y_offset + row }, L"│");
            }
            ...
            if (row < game->grid && column < game->grid) {
                Interface.draw_text((COORD) { x_offset + 2 + x_inc * 2, y_offset + 1 + y_inc }, L"%d", board[column][row]);
                
                if ((column + 1) % 3 == 0) { // Right side
